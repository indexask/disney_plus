...
def PeliculaListFilter(request,tipo):
    # Filtrar las películas con género "comedia"
    peliculas = Pelicula.objects.filter(genero=tipo)

    # Ordenar las películas por nombre
    peliculas = peliculas.order_by('nombre')

    # Pasar las películas al contexto
    contexto = {"peliculas": peliculas}

    return render(request, "peliculas_list.html", contexto)
